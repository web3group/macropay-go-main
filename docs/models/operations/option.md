...
```go
macropaygo.WithServerURL("https://api.example.com")
```
...
```go
macropaygo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
macropaygo.WithServer("my-server")
```
...
```go
macropaygo.WithClient(httpClient)
```
...
```go
macropaygo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
macropaygo.WithTimeout(30 * time.Second)
```
